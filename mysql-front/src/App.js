...
function App() {
  ...
  // useEffect(() => {기능},[특정변수명]);
   ...
   /* useEffect 최초 1회만 실행 */
  // useEffect(() => {
  // 모든유저보기(); //홈페이지 들어오면 최초 1회로 모든유저들이 보이고, 
  // },    []    ); // [] 비어있기 때문에 홈페이지가 보일 때 딱 한 번만 실행

  // useEffect [users]를 넣어주면 유저목록에 변하가 발생하면 모두 불러오기 기능 다시 실행
  useEffect(() => {
    모든유저보기(); //홈페이지 들어오면 최초 1회로 모든유저들이 보이고, 
  },    [/*users*/]    ); // 추가 삭제시 새로고침
...
/* 
 ...
 /*
  ...
  // ********** 유저추가 **********/
  //async await 사용해서 유저 추가하기 addUser 에서 가져온 user 한 명을 넣어주기
  ...
  const deleteUser = async (id) => {
    ...
    */
    await axios.delete(`/users?id=${id}`); // ``(백틱)
    // await axios.delete(`/users`); 
    /*
    자바 컨트롤러에서 @DeleteMapping("/{id}") 매개변수 = 파라미터에 (@PathValriable int id) 작성
    
    리엑트 axios에서 id=${id} 이다. 
    await axios.delete(`/users?id=${id}`);

    나중에 주소값에 id 대신 삭제할 번호가 들어갈 수 있도록 설정
    자바 컨트롤러에서 @DeleteMapping() 에 특정 id 값을 설정하지 않을 경우 
    매개변수 = 파라미터에 (@RequestParam(value="id") int id) //(value="id") = 프론트에느에서 가져온 id값
    params : {id}
    ...
    */
    ...
    user.id != id :: 유저아이디와 삭제하고자하는 id 가 일치하지 않으면
  ...
  const updateUser = async (user) => {
    ...
    //Controller에 putMapping /users로 주소값이 설정된 곳으로 연결 
    setUsers(users.map(u => (u.id === user.id ? user : u))) 
    // 수정한 유저의 id값이 일치하는지 확인하고, id값이 일치하지 않다면 
    //기존에 있던 유저 정보로 수정하지 않고 전달
  }
